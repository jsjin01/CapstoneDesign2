...
public class MultiBossMonster : MonoBehaviourPun, IPunObservable
{
    ...
    IEnumerator SkillApply() // 보스 몬스터 스킬 발동
    {
        ...
        while (skillNum > 0)
        {
            foreach (GameObject playerObject in playerObjects)
            {
                ...
                {
                    photonView.RPC("SpawnBossHand", RpcTarget.All, playerObject.transform.position - new Vector3(0, 3.7f, 0), rotation);
                }
                ...
                Debug.Log($"스킬 한번 사용 끝" );
                Debug.Log($"스킬 사용 횟수: {skillNum}" );
                skillNum--;
            }
        }
        Debug.Log($"스킬 사용 끝" );
        lastSkill = Time.time; // 스킬 종료 시점에 마지막 스킬 시간 갱신
        currentState = STATE.PATROL; // 스킬 종료 후 순찰 상태로 전환
    }

    [PunRPC]
    void SpawnBossHand(Vector3 position, Quaternion rotation)
    {
        // 마스터 클라이언트가 PhotonNetwork.Instantiate로 소유권 부여
        if (PhotonNetwork.IsMasterClient)
        {
            PhotonNetwork.Instantiate("BossHand", position, rotation);
        }
    }
